...
function Packer (props) {
  ...
  props.ignoreFiles = props.ignoreFiles || [ ".edpignore",
                                             ".gitignore",
                                             "package.json" ]
  ...
  // only do the magic bundling stuff for the lib folder that
  // lives right next to a package.json file.
  this.bundleMagic = this.parent &&
                     this.parent.packageRoot &&
                     this.basename === "lib"

  // in a lib folder, resolve symbolic links to
...
Packer.prototype.readBundledLinks = function () {
  ...
  this.pause()
  fs.readdir(this.path + "/lib", function (er, list) {
...
Packer.prototype.applyIgnores = function (entry, partial, entryObj) {
  ...
  if (entry === "lib" && this.packageRoot) return true
  ...
  // in a lib folder, we only include bundled dependencies
  // also, prevent packages in lib from being affected
...
Packer.prototype.addIgnoreFiles = function () {
  ...
  // if there's a .edpignore, then we do *not* want to
  // read the .gitignore.
  if (-1 !== entries.indexOf(".edpignore")) {
...
Packer.prototype.getChildProps = function (stat) {
  ...
  // only follow symbolic links directly in the lib folder.
  props.follow = false
  return props
}
...
Packer.prototype.emitEntry = function (entry) {
  ...
  // rename it to .edpignore in the output.
  if (entry.basename === ".gitignore") {
    entry.basename = ".edpignore"
