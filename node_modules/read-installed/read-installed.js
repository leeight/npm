...
/*
This will traverse through all lib folders,
...
for each folder in installed
  obj.dependencies[folder] = READ(packagefolder+lib+folder,
...
for each dep in obj.dependencies not in installed
  ...
  while r
    if r.dependencies[dep]
      if r.dependencies[dep].version !satisfies obj.dependencies[dep]
        WARN
...
as far as the left-most lib folder.
2. Ignore anything in lib that isn't a package folder.
...
function readInstalled (folder, depth, cb) {
  ...
  readInstalled_(folder, null, null, null, null, 0, depth, function (er, obj) {
    if (er) return cb(er)
...
var rpSeen = {}
function readInstalled_ (folder, opt_pkgfolder, parent, name, reqver, depth, maxDepth, cb) {
  var installed
    , obj
    , real
    , link

  var defPkgFolder = path.resolve(folder, "lib");
  var pkgfolder = opt_pkgfolder || defPkgFolder;
  var pkgjson = path.resolve(folder, "package.json")
  if (opt_pkgfolder) {
    pkgjson = path.resolve(opt_pkgfolder, "package.json")
  }

  fs.readdir(defPkgFolder, function (er, i) {
  ...
  fs.lstat(folder, function (er, st) {
    ...
    fs.realpath(pkgfolder, function (er, rp) {
      real = rp
  ...
  function next (er) {
    ...
    if (obj === true) {
      obj = {dependencies:{}, path:pkgfolder}
    ...
    asyncMap(installed, function (pkg, cb) {
      ...
      if (depth >= maxDepth) {
        ...
        var pkgfolder = path.resolve(folder, "lib", pkg, rv)
      ...
      readInstalled_( folder
                    , path.resolve(folder, "lib", pkg, rv)
                    , obj, pkg, rv, depth + 1, maxDepth
                    , cb )
