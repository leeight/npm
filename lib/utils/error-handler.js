...
function exit (code, noLog) {
  ...
  else rm("edp-debug.log", function () { rm(npm.tmp, reallyExit) })
...
function errorHandler (er) {
  ...
  default:
    ...
    log.error("", ["If you need help, you may report this log at:"
                  ,"    <http://github.com/ecomfe/edp/issues>"
                  ,"or email it to:"
                  ,"    <ecomfe-tech@baidu.com>"
                  ].join("\n"))
    printStack = false
    break
  }
...
function writeLogFile (cb) {
  ...
  var fs = require("graceful-fs")
    , fstr = fs.createWriteStream("edp-debug.log")
    , util = require("util")
