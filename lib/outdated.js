...
function outdated_ (args, dir, parentHas, cb) {
  // get the deps from package.json, or {<dir/lib/*>:"*"}
  // asyncMap over deps:
  //   shouldHave = cache.add(dep, req).version
  //   if has === shouldHave then
  //     return outdated(args, dir/lib/dep, parentHas + has)
  ...
  var has = null
  fs.readdir(path.resolve(dir, "lib"), function (er, pkgs) {
    if (er) {
    ...
    asyncMap(pkgs, function (pkg, cb) {
      var jsonFile = path.resolve(dir, "lib", pkg, "package.json")
