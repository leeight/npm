...
function linkStuff (pkg, folder, global, didRB, cb) {
  ...
  // if it's global, and folder is in {prefix}/lib,
...
function rebuildBundles (pkg, folder, parent, gtop, cb) {
  ...
  fs.readdir(path.resolve(folder, "lib"), function (er, files) {
    ...
    chain(files.filter(function (file) {
      ...
      file = path.resolve(folder, "lib", file)
      return function (cb) {
...
function linkBins (pkg, folder, parent, gtop, cb) {
  if (!pkg.bin || !gtop && path.basename(parent) !== "lib") {
    return cb()
  }
