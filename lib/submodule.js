...
// If there is one, then create a git submodule in the lib folder.
...
var getSubmodules = function getSubmodules (cb) {
  exec( npm.config.get("git"), ["submodule", "status"], null, false
    ...
    res = stdout.trim().split(/\n/).map(function (line) {
      ...
      // only care about submodules in the lib folder.
      return line && line.match(/^lib\//)
    }).map(function (line) {
      return line.replace(/^lib\//g, "")
    })
