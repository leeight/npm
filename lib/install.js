...
// every time we dive into a deeper lib folder, the "family" list that
...
install.usage = "edp install"
              + "\nedp install <pkg>"
              + "\nedp install <pkg>@<tag>"
              + "\nedp install <pkg>@<version>"
              + "\nedp install <pkg>@<version range>"
              + "\nedp install <folder>"
              + "\nedp install <tarball file>"
              + "\nedp install <tarball url>"
              + "\nedp install <git:// url>"
              + "\nedp install <github username>/<github project>"
              + "\n\nCan specify one or more: edp install ./foo.tgz bar@stable /some/folder"
    ...
    if (!args.length) {
      ...
      if (npm.config.get("global")) args = ["."]
        ...
        if (data.name === path.basename(where) &&
            path.basename(path.dirname(where)) === "lib") {
...
/**
 * 安装完毕之后，生成esl-config.json
 * 当然还是不能直接使用，估计需要执行一次xhr才可以。
 */
function genEslConfig(where) {
  var config = {};

  var packages = [];
  fs.readdirSync(path.resolve(where, 'lib')).forEach(function(dir){
    var versions = fs.readdirSync(path.resolve(where, 'lib', dir));
    versions.forEach(function(version){
      var pkg = {
        name: dir,
        location: 'lib/' + dir + '/' + version
      };
      var jsonFileName = path.resolve(where, 'lib', dir, version, 'package.json');
      if (fs.existsSync(jsonFileName)) {
        var pkgCfg = JSON.parse(fs.readFileSync(jsonFileName, 'utf-8'));
        if (pkgCfg['main']) {
          pkg.main = pkgCfg['main'];
        }
        packages.push(pkg);
      }
    });
  ...
  fs.writeFileSync(path.resolve(where, 'esl-config.json'),
...
function installManyTop_ (what, where, context, cb) {
  log.verbose("installManyTop_", "what,where", what, where)
  var nm = path.resolve(where, "lib")
    , names = context.explicit
            ? what.map(function (w) { return w.split(/@/).shift() })
            : []
  log.verbose("installManyTop_", "nm", nm)
  fs.readdir(nm, function (er, pkgs) {
    if (er) {
      log.verbose("installManyTop_", "er", er)
      // lib目录不存在，估计是第一次安装.
      return installMany(what, where, context, cb)
    }
    log.verbose("installManyTop_", "pkgs", pkgs)
    pkgs = pkgs.filter(function (p) {
      return !p.match(/^[\._-]/)
    })
    asyncMap(pkgs.map(function (p) {
      ...
      // if there's nothing in lib, then don't freak out.
      if (er) packages = []
      log.verbose("installManyTop_", "packages", packages)
...
function installMany (what, where, context, cb) {
  log.verbose('installMany', 'what,where', what, where)
  ...
  readDependencies(context, where, opt, function (er, data, wrap) {
    ...
    asyncMap( what
      ...
      if (targets && targets.map) {
        log.verbose('installMany', 'targets', targets.map(function(t){return t.name + '@' + t.version}).join(","))
      }
      ...
      asyncMap(targets, function (target, cb) {
        log.verbose("installOne", target._id)
...
function targetResolver (where, context, deps) {
  log.verbose("targetResolver", "where, context, deps", where, context, deps)

  var alreadyInstalledManually = context.explicit ? [] : null
    , nm = path.resolve(where, "lib")
  ...
  return function resolver (what, cb) {
    ...
    // doing `edp install foo` inside of the foo project.  Print
...
function installOne (target, where, context, cb) {
  log.verbose("installOne", "target,where", target._id, where)
...
function isIncompatibleInstallOneInProgress(target, where) {
  return target._id in installOnesInProgress &&
         installOnesInProgress[target._id].indexOf(where) !== -1
}

function installOne_ (target, where, context, cb) {
  log.verbose("installOne_", "target, where", target._id, where);

  var nm = path.resolve(where, "lib")
    , targetFolder = path.resolve(nm, target.name, target.version)
    , prettyWhere = path.relative(process.cwd(), where)
    , parent = context.parent
  ...
  if (isIncompatibleInstallOneInProgress(target, where)) {
    ...
    log.error("installOne_", "isIncompatibleInstallOneInProgress");
  ...
  if (!(target._id in installOnesInProgress)) {
    installOnesInProgress[target._id] = []
  }
  installOnesInProgress[target._id].push(where)
  var indexOfIOIP = installOnesInProgress[target._id].length - 1
  ...
  chain
    ( [ [checkEngine, target]
      ...
      , [write, target, where, targetFolder, context]
      , [postInstall, target, where, targetFolder, context] ]
    , function (er, d) {
        installOnesInProgress[target._id].splice(indexOfIOIP, 1)
...
function checkCycle (target, ancestors, cb) {
  ...
  //
  // Note: `edp install foo` inside of the `foo` package will abort
 ...
 * package安装之后的一些操作.
 */
function postInstall (target, where, targetFolder, context, cb_) {
  log.verbose("postInstall", "target,where,targetFolder", target.name, where, targetFolder);
  cb_();
}

function write (target, where, targetFolder, context, cb_) {
  log.verbose("write", "target, where", target._id, where)
  ...
  chain
    ( [ [ cache.unpack, target.name, target.version, targetFolder
      ...
      , function (cb) {
          ...
          var bd = path.resolve(targetFolder, "lib")
...
function installManyAndBuild (deps, targetFolder, context, cb) {
  log.verbose("installManyAndBuild", "deps, targetFolder", deps, targetFolder)
