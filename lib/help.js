  ...
  if (section) {
    ...
    console.log
      ( ["\nUsage: edp <command>"
        ...
        , ""
        , "edp <cmd> -h     quick help on <cmd>"
        , "edp -l           display full usage info"
        , "edp faq          commonly asked questions"
        , "edp help <term>  search for help on <term>"
        , "edp help edp     involved overview"
        , ""
        , "Specify configs in the ini-formatted file:"
        , "    " + npm.config.get("userconfig")
        , "or on the command line via: edp <command> --key value"
        , "Config info can be viewed via: edp help config"
        , ""
        , "edp@" + npm.version + " " + path.dirname(__dirname)
        ].join("\n"))
    cb(er)
  })
}
